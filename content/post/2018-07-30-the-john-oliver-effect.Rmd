...
ggplot(data = tweets.content, mapping = aes(x = Day, n, color = isRetweet)) +
  ...
  geom_line()+
  xlab("Day")+ylab("Number of Tweets with Terms")+
...
tweets.bios_threegroup <-
  ...
  mutate(ever_john = if_else(ever_john_mean == 1, "Only Mention CRISPR in conjunction John Oliver", 
                             if_else(ever_john_mean == 0, "Never Mentioned John Oliver in conjunction with CRISPR",
                                     "Mentioned CRISPR both in conjunction and otherwise"))) %>% 
  ungroup() 
...
bio_words_threegroup_source %>% 
  ...
  comparison.cloud(colors = c("#F8766D", "#00BFC4", "green"), random.order = F,
                   max.words = 200,
                   title.size = 1.5)
```
...
# characterize the information sources for one and dones ----
tweets.firstjohn <- 
  tweets.df %>% 
  filter(screenName %in% firstjohn$screenName) 


# compare the links for people who first tweeted about crispr + JO vs. others 
...
Among the 359 users who first tweeted about CRISPR in conjunction with John Oliver, the most popular tweet sources of their first tweets were TIME (63), Synthego (43), Rolling Stone (36), and the Bulletin of the Atomic Scientists (8). When we look at the source of all retweets, the same sources top the list for both groups of users (those who tweeted about CRISPR before John Oliver and those who did not).
