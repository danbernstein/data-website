...
I decided to change the mapping aesthetics, instead of usin geographic information, I used the margin of victory on the X axis, the Y axis is the number of workers affected by the shutdown, and the size of each dot is the percent of all workers in the state affected. I believe these changes still clearly highlight the states that are disproportionately affected, while also allowing better comparison. The major downside is that once you move away from the geographic map layout, the visualization becomes more difficult to immediately grasp the units (the state level). Additionally, there is substantial white space my visualization, which is not very appealing.

An interactive version is also included below.
...
p <-
  ...
  geom_segment(aes(x = -2, xend = -12, y=1000, yend=1000),  
               ...
               arrow = arrow(length = unit(0.02, "npc"),
                             type="closed"),
               size = 1, colour = "blue")+
  annotate("text", x = -18, y = 1000, label = "clinton", colour = "blue", size = 6)+
  geom_segment(aes(x = 2, xend = 12, y=1000, yend=1000),  
               lineend = 'round', linejoin = 'mitre',
               arrow = arrow(length = unit(0.02, "npc"),
                             type="closed"),
               size = 1, colour = "red")+
  annotate("text", x = 18, y = 1000, label = "trump", colour = "red", size = 6)+  
  ...
  labs(x = "margin of victory in 2016 election (% popular vote)", 
       ...
       subtitle = "size is total number of furloughed employees")+
  ylim(0, 1000)
```

```{r, echo = F, warning=F, out.width="80%", out.extra='keepaspectratio', fig.align='center'}
p
```

```{r, echo = F, warning=F, out.width="80%", out.extra='keepaspectratio'}
library(shiny)
div(plotly::ggplotly(p, tooltip=c("text")), align = 'center')
```